#!/bin/sh
# Manage XRAY-P2P client lifecycle (OpenWrt)
...
if ! xray_common_try_source \
    "${XRAY_CLIENT_INSTALL_LIB:-scripts/lib/client_install.sh}" \
    "scripts/lib/client_install.sh" \
    "lib/client_install.sh"; then
    xray_die "Unable to load client install library."
fi

if ! xray_common_try_source \
    "${XRAY_CLIENT_REMOVE_LIB:-scripts/lib/client_remove.sh}" \
    "scripts/lib/client_remove.sh" \
    "lib/client_remove.sh"; then
    xray_die "Unable to load client remove library."
fi
