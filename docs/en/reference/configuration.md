...
The schema files are generated from the persisted Go configuration models. Run `make schema` after changing those models, `make schema-check` to detect drift without changing files, `make schema-test` to validate schemas and fixtures, and `make schema-compat` to prove that versioned legacy Desired fixtures remain valid in both the schema and runtime decoders. Do not edit files under `schemas/` manually.
