...
Когда TUN включён, это также установит маршрут ОС для CIDR (если не использовать `--no-routes`).

Если сервис запущен и Live-конфигурация Xray доступна, команда применяет
правило маршрутизации Xray через API времени работы и согласует маршрут ОС в
том же потоке. Она не перезапускает `xray-core` и не создаёт `apply.request`
только ради изменения маршрута. Если сервис остановлен, изменение сохраняется в
желаемые входные данные, а маршрут применяется при следующем старте сервиса или
`xp2p run`.
