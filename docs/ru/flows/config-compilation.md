...
входные данные согласованными без создания `apply.request`.

Большинство изменений уровня ОС, таких как TUN, DNS, файрвол и nftables,
остаётся во владении сервиса. Если команда CLI, способная применять изменения
во время работы, имеет немедленный побочный эффект в маршрутах ОС, она всё равно
должна завершить изменение маршрута в том же потоке после успешного применения
через API Xray и записи соответствующих Desired/Live-артефактов. Основной
пример — изменения CIDR-редиректов: после применения и проверки правила
маршрутизации через API Xray команда напрямую согласует соответствующий
разделённый маршрут ОС. Она не должна создавать `apply.request` или
перезапускать `xray-core` только ради обновления такого маршрута. Если
согласование маршрута завершается ошибкой, команда возвращает явную ошибку, а
не прячет устаревшее состояние ОС за отложенным перезапуском.
