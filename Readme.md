...
- Tray controller `xp2p-ui` ships with the MSI and auto-starts via the current user's Run key; disable with `XP2P_UI_AUTOSTART=0`.
...
`xp2p` reads configuration in the following order: built-in defaults > optional file(s) > environment variables > CLI overrides. By default it loads `xp2p-client.toml` and `xp2p-server.toml` from the platform install root (for example `/etc/xp2p` on Linux packages, the MSI install root on Windows); override with `XP2P_CONFIG_ROOT` or pass `--config path/to/file`. Settings map 1:1 to environment variables via the `XP2P_` prefix (`XP2P_SERVER_INSTALL_DIR`, `XP2P_CLIENT_SERVER_ADDRESS`, etc.). See `config_templates/xp2p.example.yaml` for a starting point:
