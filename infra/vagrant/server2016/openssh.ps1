...
$script:OpenSshBinDir = Join-Path $env:SystemRoot "System32\OpenSSH"
$programFilesOpenSsh = Join-Path $env:ProgramFiles "OpenSSH"
if (-not (Test-Path (Join-Path $script:OpenSshBinDir "sshd.exe")) -and (Test-Path (Join-Path $programFilesOpenSsh "sshd.exe"))) {
    $script:OpenSshBinDir = $programFilesOpenSsh
}
$script:OpenSshRestartRequired = $false
$script:OpenSshCapabilitiesAvailable = $true
...
function Ensure-OpenSshCapability {
    ...
    foreach ($cap in $caps) {
        try {
            ...
            $script:OpenSshCapabilitiesAvailable = $false
            Write-Info ("Get-WindowsCapability failed; will use zip fallback if needed. Error: {0}" -f $_.Exception.Message)
            return
        }
...
function Install-OpenSshFromZip {
    $zipUrl = $env:XP2P_OPENSSH_ZIP_URL
    if (-not $zipUrl) {
        $zipUrl = "https://github.com/PowerShell/Win32-OpenSSH/releases/latest/download/OpenSSH-Win64.zip"
    }

    $tempZip = Join-Path $env:TEMP "OpenSSH-Win64.zip"
    $tempDir = Join-Path $env:TEMP "OpenSSH-Win64"
    if (Test-Path $tempDir) {
        Remove-Item $tempDir -Recurse -Force
    }

    Write-Info ("Downloading OpenSSH from {0}" -f $zipUrl)
    try {
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    }
    catch {
        Write-Info ("Failed to set TLS 1.2 for download: {0}" -f $_.Exception.Message)
    }
    ...
    try {
        ...
        throw ("OpenSSH download failed. Set XP2P_OPENSSH_ZIP_URL to an internal mirror if needed. Error: {0}" -f $_.Exception.Message)
    }

    Expand-Archive -Path $tempZip -DestinationPath $tempDir -Force
    $sourceDir = Join-Path $tempDir "OpenSSH-Win64"
    if (-not (Test-Path $sourceDir)) {
        $candidate = Get-ChildItem -Path $tempDir -Directory | Select-Object -First 1
        if ($candidate) {
    ...
    if (-not (Test-Path $sourceDir)) {
        throw "OpenSSH archive extraction failed; no files found in $tempDir."
    }
    ...
    $script:OpenSshBinDir = $installDir
    $script:OpenSshRestartRequired = $true
}

function Ensure-SshdRegistered {
    $systemExe = [System.IO.Path]::Combine($env:SystemRoot, "System32\OpenSSH\sshd.exe")
    $programExe = [System.IO.Path]::Combine($env:ProgramFiles, "OpenSSH\sshd.exe")
    ...
    if ($service -and $service.PathName) {
        ...
        if ($normalizedPath -like "*$systemExe*" -or $normalizedPath -like "*$programExe*") {
    ...
    if (-not (Test-Path $installScript)) {
        if ($script:OpenSshCapabilitiesAvailable) {
        ...
        if (-not (Test-Path $installScript)) {
            Write-Info "System Install-SSHD.ps1 still missing. Installing OpenSSH from zip."
            Install-OpenSshFromZip
            $installScript = Join-Path $script:OpenSshBinDir "install-sshd.ps1"
            if (-not (Test-Path $installScript)) {
                Write-Info "install-sshd.ps1 missing after zip install. Using manual sshd service registration."
...
function Repair-HostKeyPermissions {
    ...
    & icacls $KeyPath /setowner "Administrators" | Out-Null
    & icacls $KeyPath /grant:r "SYSTEM:F" "Administrators:F" | Out-Null
    & icacls $KeyPath /remove:g "Users" "Authenticated Users" "Everyone" "BUILTIN\Users" | Out-Null
    $localUser = "{0}\vagrant" -f $env:COMPUTERNAME
    & icacls $KeyPath /remove:g $localUser | Out-Null
...
function Ensure-SshdConfig {
    ...
    $exePath = Join-Path $script:OpenSshBinDir "sshd.exe"
    if (-not (Test-Path $exePath)) {
        $fallback = Join-Path $env:ProgramFiles "OpenSSH\sshd.exe"
        if (Test-Path $fallback) {
            $script:OpenSshBinDir = Split-Path $fallback
            $exePath = $fallback
        }
        else {
...
function Ensure-SshdServiceImagePath {
    $serviceReg = "HKLM:\SYSTEM\CurrentControlSet\Services\sshd"
    if (-not (Test-Path $serviceReg)) {
        return $false
    }
    ...
    $logDir = Join-Path $SshConfigDir "logs"
    if (-not (Test-Path $logDir)) {
        New-Item -ItemType Directory -Path $logDir -Force | Out-Null
    }

    $configPath = Join-Path $SshConfigDir "sshd_config"
    $logPath = Join-Path $logDir "sshd.log"
    $expected = "`"$exePath`" -f `"$configPath`" -E `"$logPath`""

    $current = $null
    try {
        $props = Get-ItemProperty -Path $serviceReg -Name "ImagePath" -ErrorAction Stop
        $current = $props.ImagePath
    }
    catch {
        $current = $null
    }

    if ($current -and ($current.Trim()) -eq $expected) {
        return $false
    }

    Write-Info ("Updating sshd ImagePath to enable file logging: {0}" -f $logPath)
    Set-ItemProperty -Path $serviceReg -Name "ImagePath" -Value $expected -Force | Out-Null
    return $true
}
...
function Ensure-VagrantKeys {
    ...
    $adminKeysPath = Join-Path $SshConfigDir "administrators_authorized_keys"
    $adminExisting = @()
