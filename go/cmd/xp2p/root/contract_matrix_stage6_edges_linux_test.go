...
func TestStage6DNSControlCharactersUseJSONEscaping(t *testing.T) {
	...
	for _, role := range []string{"client", "server"} {
		for _, item := range []struct {
			...
			{"add", []string{role, "dns-forward", "add", "--domain", "control.example", "--target", "192.0.2.1:53"}, "labels"},
			{"list", []string{role, "dns-forward", "list"}, "entries"},
			{"remove", []string{role, "dns-forward", "remove", "--domain", "control.example"}, "domains"},
		} {
			path := "xp2p " + role + " dns-forward " + item.action
			execution := executeContractCase(item.args, false)
			result := stage6ResultMap(t, path, execution)
			if !strings.Contains(execution.stdout, `\n\t\u0001`) ||
				strings.Contains(execution.stdout, "forward:\n") {
				t.Fatalf("%s did not JSON-escape control characters: %q", path, execution.stdout)
			}
			encoded, err := json.Marshal(result[item.key])
			if err != nil || !strings.Contains(string(encoded), `\n\t\u0001`) {
				t.Fatalf("%s lost control characters: value=%#v err=%v", path, result[item.key], err)
			}
		}
	}
}
