...
func TestRun_RollbackAndApplyErrorOnEarlyFailureAfterPendingApply(t *testing.T) {
	...
	lkgConfig := config.LkgConfigPath(layout.ClientConfigFileName)
	lkgConfigData := []byte("[client]\n")
	...
	appliedStatePath := config.ConfigPath(layout.ClientAppliedStateFileName)
	...
	pendingConfig := config.PendingConfigPath(layout.ClientConfigFileName)
	...
	if marker.Role != apply.RoleClient {
	...
	liveConfig := config.LiveConfigPath(layout.ClientConfigFileName)
