...
func newClientInstallCmd(cfg commandConfig) *cobra.Command {
	...
	flags.StringP("user", "u", "", "user email (used to derive the <user><host>.rev reverse bridge)")
	flags.StringP("password", "w", "", "user password")
	flags.StringP("sni", "s", "", "TLS server name (SNI)")
	flags.StringP("link", "L", "", "client connection link")
...
func newClientDeployCmd(cfg commandConfig) *cobra.Command {
	...
	flags.StringP("user", "u", "", "user identifier (email)")
	flags.StringP("password", "w", "", "user password (auto-generated when omitted)")
	flags.StringP("trojan-port", "T", "", "service port")
	flags.StringP("mode", "M", "", "target client mode (proxy or tun; also supports tun:split or tun:full)")
	flags.StringP("tun-mode", "m", "", "TUN routing mode (split or full)")
