...
func runClientForwardList(ctx context.Context, cfg config.Config, args []string) int {
	...
	if clioutput.EnabledContext(ctx) {
		...
		if err := clioutput.SetResultContext(ctx, result); err != nil {
			logging.Error("xp2p client forward list: publish JSON result failed", "err", err)
			return 1
		}
		return 0
	}
