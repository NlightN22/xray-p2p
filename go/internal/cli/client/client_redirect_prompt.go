...
func resolveClientBinding(req clientBindingRequest) (redirect.Binding, error) {
	var (
		candidates []redirect.Binding
		err        error
	)
	if req.Matching {
		candidates, err = listClientRedirectBindings(req.InstallDir, req.ConfigDir, req.CIDR, req.Domain)
	} else {
		candidates, err = listClientEndpointBindings(req.InstallDir, req.ConfigDir)
	}
	if err != nil {
...
func listClientEndpointBindings(installDir, configDir string) ([]redirect.Binding, error) {
...
func listClientRedirectBindings(installDir, configDir, cidr, domain string) ([]redirect.Binding, error) {
	target, err := redirect.ResolveRule(cidr, domain)
	if err != nil {
		return nil, err
	}
	records, err := clientRedirectListFunc(client.RedirectListOptions{
		InstallDir: installDir,
		ConfigDir:  configDir,
		Pending:    true,
	})
	...
	bindings := make([]redirect.Binding, 0, len(records))
	for _, rec := range records {
		if !clientRedirectRecordMatches(rec, target) {
			continue
		}
		bindings = append(bindings, redirect.Binding{
			Tag:  rec.Tag,
...
func clientRedirectRecordMatches(rec client.RedirectRecord, target redirect.Target) bool {
	switch target.Kind {
...
func clientBindingRequiredError(err error) bool {
