...
from tests.host.win.flows import deploy

HEARTBEAT_STATE_FILES = deploy_assert.HEARTBEAT_STATE_FILES
...
def start_deploy_tunnel(
    ...
    *,
    ...
    client_proc = deploy.start_client_deploy(
        client_host,
    ...
    initial_server_log = deploy.wait_for_any_log_phrase(
        server_host,
        server_proc,
        [
    ...
    server_status = deploy.wait_for_any_log_phrase(
        server_host,
        server_proc,
        [
    ...
    if server_status == "server deploy: xray-core start failed":
        combined = deploy.read_combined_logs(server_host, server_proc)
