...
def test_dns_forward_server_add_and_remove(openwrt_server_host, openwrt_client_host, xp2p_on_both):
    ...
    try:
        ...
        _reset_server_state(openwrt_server_host)

        add = openwrt_server_host.run(
            f"/usr/bin/xp2p server dns-forward add --domain {DOMAIN} --target {client_ip}:53 --with-forward --intercept --quiet"
        )
        assert add.rc == 0, f"add command failed: {add.stderr}"

        dhcp_show = openwrt_server_host.run("uci show dhcp")
        assert dhcp_show.rc == 0
        _assert_dns_server_entry(dhcp_show.stdout or "", DOMAIN)
        ...
        forward_port = _detect_forward_port(openwrt_server_host, client_ip)
        service_pid = _start_server_service(openwrt_server_host)
        _wait_for_port(openwrt_server_host, forward_port)

        lookup_direct = openwrt_server_host.run(f"nslookup {DOMAIN} 127.0.0.1:{forward_port} || true")
        ...
        lookup_intercept = openwrt_server_host.run(f"nslookup {DOMAIN} 127.0.0.1 || true")
        ...
        remove = openwrt_server_host.run(
            f"/usr/bin/xp2p server dns-forward remove --domain {DOMAIN} --intercept --quiet"
        )
        ...
        _stop_server_service(openwrt_server_host, service_pid)
...
def _reset_server_state(host) -> None:
...
def _assert_dns_server_entry(output: str, domain: str) -> None:
...
def _start_server_service(host):
    start = host.run(
        "nohup /usr/bin/xp2p server service run --heartbeat=false --max-restarts=0 >/tmp/xp2p-server.log 2>&1 & echo $!"
    )
    assert start.rc == 0, f"failed to start server service: {start.stderr}"
    return (start.stdout or "").strip()


def _stop_server_service(host, pid):
    if not pid:
        return
