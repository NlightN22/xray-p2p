...
$commandLine = "`"$xp2p`" server run --quiet --path `"$installDir`" --config-dir `"$configDir`" --xray-log-file `"$logRelative`""
...
$deadline = (Get-Date).AddSeconds({SERVER_RUN_STABILIZE_SECONDS})
